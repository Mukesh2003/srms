...
#         password=request.form['password']
#     #     # cursor=mydb.cursor(buffered='True')
#     #     # cursor.execute('select * from faculty where facno=%s and password=%s',(userid,password))
#     #     # user=cursor.fetchone()
#     #     if user is not None:
#     #         session['loggedin']=True
#     #         session['username']=user
            
#     #         return  render_template('dashfac.html')                 #'''redirect(url_for('dashboard'))'''
#     #     else:
#     #         return render_template('logf.html',error="Incorrect username or password")
...
#@app.route('/dashf')
#def dashf():
 #   return render_template('dashfac.html')
